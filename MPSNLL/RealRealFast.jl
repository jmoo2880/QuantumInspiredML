...
function generate_starting_mps(chi_init::Int, site_indices::Vector{Index{Int64}}; 
    num_classes::Int = 2, random_state = nothing)
    """Generate the starting weight MPS, W, using values sampled from 
    a standard 0 centred Gaussian. Accepts a chi_init paramter which specifies
    the initial (uniform) bond dimension of the mps"""

    if random_state !== nothing 
        Random.seed!(random_state)
        println("Generating initial weight MPS with bond dimension χ = $chi_init")
        println("Using random state: $random_state")
    else
        println("Generating initial weight MPS with bond dimension χ = $chi_init")
    end

    W = randomMPS(ComplexF64, site_indices; linkdims=chi_init)
    ...
    new_site = randomITensor(ComplexF64, new_site_idxs)
    ...
    # bring the MPS into canonical form by making all tensors 1,...,j-1 left orthogonal
    orthogonalize!(W, length(W))

    return W

end

function construct_caches(mps::MPS, training_pstates::timeSeriesIterable; going_left=true)
    """Function to pre-compute tensor contractions between the MPS and the product state"""
    ...
    if going_left
        ...
        for i in 1:N_train
            LE[1, i] = conj(training_pstates[i].pstate[1]) * mps[1]
        end
        ...
        # going right
        for i in 1:N_train
            RE[N, i] = conj(training_pstates[i].pstate[N]) * mps[N]
        end
...
function compute_yhat_and_derivative(BT::ITensor, LEP::PCacheCol, REP::PCacheCol, 
    ...
    phi_tilde = conj(ps[lid]) * conj(ps[rid]) # phi tilde 

    if lid == 1
        # at the first site, no LE
        # formatted from left to right, so env - product state, product state - env
        phi_tilde = conj(ps[lid]) * conj(ps[rid]) * REP[rid+1]
    elseif rid == length(ps)
        # terminal site, no RE
        phi_tilde = LEP[lid-1] * conj(ps[lid]) * conj(ps[rid]) 
    else
        # we are in the bulk, both LE and RE exist
        phi_tilde = conj(ps[lid]) * conj(ps[rid]) * LEP[lid-1] * REP[rid+1]
    end
...
function loss_grad_per_sample(BT_c::ITensor, LEP::PCacheCol, REP::PCacheCol,
    product_state::PState, lid::Int, rid::Int)
    ...
    yhat, phi_tilde = compute_yhat_and_derivative(BT_c, LEP, REP, product_state, lid, rid)
...
function loss_and_gradient(BT::ITensor, LE::PCache, RE::PCache,
    ...
    loss,grad = Folds.mapreduce((LEP,REP, prod_state) -> loss_grad_per_sample(BT_c,LEP,REP,prod_state,lid,rid),+, eachcol(LE), eachcol(RE),ϕs)
...
function apply_update(BT_init::ITensor, LE::PCache, RE::PCache, lid::Int, rid::Int,
    ...
    normalize!(bt_re)
    lg = x -> loss_and_gradient(x, LE, RE, ϕs, lid, rid, C_index)
...
function decompose_bond_tensor(BT::ITensor, lid::Int, rid::Int; 
    chi_max=nothing, cutoff=nothing, going_left=true)
...
function update_caches!(left_site_new::ITensor, right_site_new::ITensor, 
    ...
    if going_left
        for i = 1:num_train
            if rid == num_sites
                RE[num_sites,i] = right_site_new * conj(product_states[i].pstate[num_sites])
            else
...
function fit_mps(X_train::Matrix, y_train::Vector, X_val::Matrix, 
    y_val::Vector, X_test, y_test; chi_init=4, nsweep=5, chi_max=25, cutoff=1E-10, 
    ...
    training_states = generate_all_product_states(X_train_scaled, y_train, sites)
    validation_states = generate_all_product_states(X_val_scaled, y_val, sites)
    testing_states = generate_all_product_states(X_test_scaled, y_test, sites)

    # generate the starting MPS with unfirom bond dimension χ_init and random values (with seed if provided)
    num_classes = length(unique(y_train))
    W = generate_starting_mps(chi_init, sites; num_classes=num_classes, random_state=random_state)
    ...
    for itS = 1:nsweep
        ...
        for j = 1(length(sites)-1):-1:1
            ...
            new_BT = apply_update(BT, LE, RE, j, (j+1), training_states; iters=update_iters, verbosity=verbosity) # optimise bond tensor
            # decompose the bond tensor using SVD and truncate according to χ_max and cutoff
            lsn, rsn = decompose_bond_tensor(new_BT, j, (j+1); chi_max=chi_max, cutoff=cutoff, going_left=true)
        ...
        for j = 1:(length(sites)-1)
            #print("Bond $j")
            BT = W[j] * W[(j+1)]
            new_BT = apply_update(BT, LE, RE, j, (j+1), training_states; iters=update_iters, verbosity=verbosity)
            lsn, rsn = decompose_bond_tensor(new_BT, j, (j+1); chi_max=chi_max, cutoff=cutoff, going_left=false)
