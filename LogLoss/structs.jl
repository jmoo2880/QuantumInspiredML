...
@with_kw struct Options
    ...
    algorithm::String # OPC or PBC
    random_walk_seed::Integer # seed for random_walk algorithm
end
...
function Options(; nsweeps=5, chi_max=25, cutoff=1E-10, update_iters=10, verbosity=1, dtype::DataType=ComplexF64, loss_grad=loss_grad_KLD, bbopt=BBOpt("CustomGD"),
    ...
    encode_classes_separately::Bool=train_classes_separately, return_encoding_meta_info=false, minmax=true, algorithm="OBC", random_walk_seed=420)
    ...
    Options(nsweeps, chi_max, cutoff, update_iters, verbosity, dtype, loss_grad, bbopt, track_cost, eta, rescale, d, aux_basis_dim, encoding, train_classes_separately, 
        encode_classes_separately, return_encoding_meta_info, minmax, algorithm, random_walk_seed)
end
