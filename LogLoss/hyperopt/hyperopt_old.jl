...
GridSearch(;etas=[0.1], ds=[2,3,4], chi_maxs=[15,20,25,30,35]) = Gridsearch(etas, max_sweeps, ds, chi_maxs)

struct HGradientDescent <: SearchMethod 
    ...
    chi_step::Integer
    max_sweeps::Integer 
    max_grad_steps::Number
    use_backtracking_linesearch::Bool
end

function HGradientDescent(; 
    ...
    return HGradientDescent(eta_init, eta_range, deta_perc, min_eta_eta, eta_eta_init, min_eta_step, d_init, d_range, d_step,chi_max_init, chi_max_range, chi_step, max_sweeps, max_grad_steps, use_backtracking_linesearch)
end

struct OptimSearch <: SearchMethod end
...
hyperopt(enc::Encoding, args...; kwargs...) = hyperopt(GridSearch(), enc, args...; kwargs...) 


function hyperopt(search::SearchMethod, encoding::Encoding, Xs::AbstractMatrix, ys::AbstractVector; 
    etas::AbstractVector{<:Number}, 
    max_sweeps::Integer, 
    ds::AbstractVector{<:Integer}, 
    chi_maxs::AbstractVector{<:Integer}, 
    chi_init::Integer=4,
    train_ratio=0.9,
    force_complete_crossval::Bool=true, # override train_ratio
    ...
    pstr = e_str*tstring*encoding.name * "_" * vstring * string(search)* sig_str*"$(nfolds)fold_r$(mps_seed)_eta$(repr_vec(etas))_ns$(max_sweeps)_chis$(repr_vec(chi_maxs))_ds$(repr_vec(ds))"
    ...
    # resume if possible
    load_hyperopt!(search, logpath; logfile=logfile, resfile=resfile, finfile=finfile)
...
function hyperopt(::OptimSearch, encoding::Encoding, Xs::AbstractMatrix, ys::AbstractVector; 
    eta_init::Real=0.01, # if you want eta to be a complex number, fix the indexing for complex numbers in hyperUtils.eta_to_index()
    eta_range::AbstractBounds{<:Real} = (0.001,0.5),
    ...
    chi_init::Integer=4,
    train_ratio=0.9,
    force_complete_crossval::Bool=true,
    nfolds::Integer= force_complete_crossval ? round(Int, ceil(1 / (1-train_ratio); digits=5)) : 1, # you can use this to override the number of folds you _should_ use, but don't. You need the round for floating point reasons
    mps_seed::Real=4567,
    kfoldseed::Real=1234567890, # overridden by the rng parameter
    foldrng::AbstractRNG=MersenneTwister(kfoldseed),
    ...
    tstring = encoding.istimedependent ? "TD_" : "NTD_"
    vstring = train_classes_separately ? "Split_train_" : ""
    pstr = tstring*encoding.name * "_" * vstring * "OPT_$(nfolds)fold_r$(mps_seed)_eta$(eta_range)_ns$(max_sweeps)_chis$(chi_max_range)_ds$(d_range)"
    ...
    function acc_step!(eta_var::Number, results, nfolds::Integer, max_sweeps::Integer, chi_max::Integer, d::Integer, e::Encoding)
        if eta_var < eta_range[1] || eta_var > eta_range[2]
            return 0

        else
            return -eval_gridpoint!(results, keys(results), nfolds, max_sweeps, eta_var, chi_max, d, e)
        end
    ...
    eta, chi_max, d, e = eta_init, chi_max_init, d_init, encoding
    acc = eval_gridpoint!(results, keys(results), nfolds, max_sweeps, eta, chi_max, d, e)
    ...
    while !finished && nsteps < max_grad_steps
        ...
        acc_eta = eta_var -> acc_step!(first(eta_var), results, nfolds, max_sweeps, chi_max, d, e)

        res = Optim.optimize(acc_eta, [eta], NelderMead())
        eta_new = first(Optim.minimizer(res))
