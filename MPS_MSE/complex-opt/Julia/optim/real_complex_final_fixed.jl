...
function sample_to_product_state(ts::Vector, site_inds::Vector{Index{Int64}})
    ...
    product_state = MPS([ITensor(complex_feature_map(ts[i]), site_inds[i]) for i in eachindex(site_inds)])
...
function get_predictions(mps0::MPS, mps1::MPS, pss::Vector{PState})
    ...
    for i in eachindex(pss)
        ...
        for j in eachindex(mps0)
            overlap_mps0 *= mps0[j] * conj(ps[j])
            overlap_mps1 *= mps1[j] * conj(ps[j])
        end
...
function train_mps()
    ...
    # generate dataset
    #(X_train, y_train), (X_test, y_test) = generate_toy_timeseries(num_sites, 1000; plot_examples=true)
