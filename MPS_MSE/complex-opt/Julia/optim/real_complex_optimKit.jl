...
function basic_sweep(num_sweeps::Int, χ_max::Int=10; invert_labels=false)
    ...
    s = siteinds("S=1/2", 96) # make sure both mps optimisations use same site indices
    ...
    if invert_labels
        ...
        y_train_final = [remap_labels[label] for label in y_train_final]
...
function classify_ecg()
    # simple function that calls the basic sweep
    # first train to maximise overlap with original class 1
    mps1, all_pstates1, all_test_pstates1, loss_per_sweep1, acc_per_sweep1 = basic_sweep(5, 30)
    normalize!(mps1)
    # now invert the lable so that the overlap is maximised for original class 0 (which is now class 1)
    mps0, all_pstates0, all_test_pstates0, loss_per_sweep0, acc_per_sweep0 = basic_sweep(5, 30; invert_labels=true)
    normalize!(mps2)

    # now get preds
    overlaps_class_1 = []
    for ps in all_test_pstates1
        res = 1
        for i in eachindex(mps1)
            res *= mps1[i] * ps.pstate[i]
        end
        res = abs(res[])
        push!(overlaps_class_1, res)
    end

    overlaps_class_0 = []
    for ps in all_test_pstates0
        res = 1
        for i in eachindex(mps0)
            res *= mps0[i] * ps.pstate[i]
        end
        res = abs(res[])
        push!(overlaps_class_0, res)
    end

    preds = []
    for i in 1:100
        # compare overlaps with mps overlapping with class 1 and class 0
        if overlaps_class_1[i] > overlaps_class_0[i]
            push!(preds, 1)
        else
            push!(preds, 0)
        end
    ...
    y_true = [x.label for x in all_test_pstates1]
    acc = sum(y_true .== preds)/length(preds)

    return acc, mps1, mps2
...
#     # overlap maximised with original class 1
#     mps_c1, all_pstates, all_test_pstates, loss_per_sweep, acc_per_sweep = basic_sweep(X_train_scaled, y_train_final, X_test, y_test,s, 10, 30)

#     # overlap maximised with original class 0
#     # invert the labels so 1 -> 0 and 0 -> 1
#     remap_labels = Dict(0=>1, 1=>0)
#     y_train_flipped = [remap_labels[label] for label in y_train_final]
#     y_test_flipped = [remap_labels[label] for label in y_test]
#     mps_c0, _, _, _, _ = basic_sweep(X_train_scaled, y_train_flipped, X_test, y_test_flipped,s, 10, 30)

#     println("Norm c1: $(norm(mps_c1))")
#     println("Norm c0: $(norm(mps_c0))")

#     # normalise both MPS
#     mps_c1_regular = mps_c1
#     mps_c1_normed = normalize(deepcopy(mps_c1))
    
#     mps_c0_regular = mps_c0
#     mps_c0_normed = normalize(deepcopy(mps_c0))

#     # now make predictions - if overlap is larger for c1 then label as 1, else label as c0
#     correct_regular = 0
#     correct_normed = 0

#     for pstate in all_test_pstates
#         ps = pstate.pstate
#         y_true = pstate.label
#         # get overlap with both mps
#         res_c0_regular = 1
#         res_c0_normed = 1
#         res_c1_regular = 1
#         res_c1_normed = 1
#         for i in eachindex(ps)
#             # get overlaps for both normed and un-normed versions
#             res_c1_regular *= mps_c1_regular[i] * ps[i]
#             res_c1_normed *= mps_c1_normed[i] * ps[i]

#             res_c0_regular *= mps_c0_regular[i] * ps[i]
#             res_c1_normed *= mps_c0_normed[i] * ps[i]

#         end

#         overlap_c0_regular = abs(res_c0_regular[])
#         overlap_c0_normed = abs(res_c0_normed[])
#         overlap_c1_regular = abs(res_c1_regular[])
#         overlap_c1_normed = abs(res_c1_normed[])

#         # do un-normed (regular) first

#         y_pred_regular = 0
#         y_pred_normed = 0

#         if overlap_c1_regular > overlap_c0_regular
#             y_pred_regular = 1
#         end

#         if overlap_c1_normed > overlap_c0_normed
#             y_pred_normed = 1
#         end

#         if y_pred_regular == y_true
#             correct_regular += 1
#         end

#         if y_pred_normed == y_true
#             correct_normed += 1
#         end

#     end

#     acc_regular = correct_regular/length(all_test_pstates)

#     acc_normed = correct_normed/length(all_test_pstates)

#     println("Acc Un-Normalised: $acc_regular")
#     println("Acc Normalised: $acc_normed")

#     # return accuracies as well as norm of un-normalised mps
#     return acc_regular, acc_normed, norm(mps_c0), norm(mps_c1)
...
# bt_combined_real_imag = bt_real + bt_imag
# compute_yhat_and_derivative(bt_combined_real_imag, LE, RE, pstates[1], 1, 2)
# out = loss_and_grad_batch(bt_combined_real_imag, LE, RE, pstates, 1, 2)

#out = apply_update(bt, LE, RE, 1, 2, pstates)


#compute_yhat_and_derivative(bt, LE, RE, pstates[1], 1, 2)
# # loss and gradient will need to accept a combined, all real BT with the label index
# # compute loss and gradient per sample will also need to do the same
# # com0ute yhat and derivative as well 
# function loss_and_gradient_per_sample(BT::ITensor, LE::Matrix, RE::Matrix, ps::PState, 
#     lid::Int, rid::Int)
...
# function LossAndGradient(BT::ITensor, LE::Matrix, RE::Matrix, pss::Vector{PState},
#     lid::Int, rid::Int)
# end