...
function any_interpolate_median(
    ...
    if NN_baseline
        ts, _... = any_impute_single_timeseries_noplots(fcastable, which_class, which_sample, which_sites, :nearestNeighbour; kwargs...)
...
function any_impute_single_timeseries_noplots(
    ...
    ts, wmad, target = any_impute_single_timeseries_noplots(fcastable, which_class, which_sample, which_sites, :ITS; kwargs...)
    ...
    if method == :directMean        
        if fcast.opts.encoding.istimedependent
            ts, pred_err = any_interpolate_directMean_time_dependent(mps, fcast.opts, fcast.enc_args, target_timeseries, which_sites, kwargs...)
        else
            ts, pred_err = any_interpolate_directMean(mps, fcast.opts, fcast.enc_args, target_timeseries, which_sites, kwargs...)
        end
    elseif method == :directMedian
        if fcast.opts.encoding.istimedependent
            ...
            ts, pred_err = any_interpolate_directMedian(mps, fcast.opts, fcast.enc_args, target_timeseries, states, which_sites; dx=dx, mode_range=mode_range, xvals=xvals, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it, mode_index=mode_index, kwargs...)
        end
    elseif method == :directMode
        if fcast.opts.encoding.istimedependent
            # xvals_enc = [get_state(x, opts) for x in x_vals]

            ts = any_interpolate_directMode_time_dependent(mps, fcast.opts, fcast.enc_args, target_timeseries, which_sites, kwargs...)
        else
            sites = siteinds(mps)
            
            states = MPS([itensor(fcast.opts.encoding.encode(t, fcast.opts.d, fcast.enc_args...), sites[i]) for (i,t) in enumerate(target_timeseries)])
            ts = any_interpolate_directMode(mps, fcast.opts, fcast.enc_args, target_timeseries, states, which_sites; dx=dx, mode_range=mode_range, xvals=xvals, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it, mode_index=mode_index, max_jump=max_jump, kwargs...)
        end
    elseif method == :MeanMode
        if fcast.opts.encoding.istimedependent
            ...
            states = MPS([itensor(fcast.opts.encoding.encode(t, fcast.opts.d, fcast.enc_args...), sites[i]) for (i,t) in enumerate(target_timeseries)])
            ts = any_interpolate_MeanMode(fcast.mps, fcast.opts, target_timeseries, states, which_sites; dx=dx, mode_range=mode_range, xvals=xvals, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it, mode_index=mode_index, max_jump=max_jump, kwargs...)
        end
    ...
    elseif method == :ITS
        if fcast.opts.encoding.istimedependent
            ...
            states = MPS([itensor(fcast.opts.encoding.encode(t, fcast.opts.d, fcast.enc_args...), sites[i]) for (i,t) in enumerate(target_timeseries)])
            ts = any_interpolate_ITS_single(mps, fcast.opts, fcast.enc_args, target_timeseries, states, which_sites; kwargs...)
        end
    
    elseif method ==:nearestNeighbour
        ts = NN_interpolate(fcastable, which_class, which_sample, which_sites; X_train, y_train, n_ts=1)[1] # Does not take kwargs!!
        ...
        error("Invalid method. Choose :directMean (Expect/Var), :directMode, :directMedian, nearestNeighbour, etc")
    end
