...
function get_state(x::Float64, basis::Basis, d::Int)
    """Get the state using the feature map encoding of choice"""
    state = basis.encode(x, d)
    return state
end
...
function get_conditional_probability(x::Float64, rdm::Matrix, basis::Basis, d::Int)
    ...
    """
    # get σ_k = |⟨x_k | ρ | x_k⟩|
    state = get_state(x, basis, d)
    return abs(state' * rdm * state)

end

function get_normalisation_constant(rdm::Matrix, basis::Basis, d::Int)
    ...
    prob_density_wrapper(x) = get_conditional_probability(x, rdm, basis, d)
    # integrate over data domain xk 
    lower, upper = basis.range
    Z, _ = quadgk(prob_density_wrapper, lower, upper)
    return Z
end

function get_cdf(x::Float64, rdm::Matrix, Z::Float64, basis::Basis, d::Int)
    ...
    value i.e., F(x)."""
    prob_density_wrapper(x_prime) = (1/Z) * get_conditional_probability(x_prime, rdm, basis, d)
    lower, _ = basis.range
    cdf_val, _ = quadgk(prob_density_wrapper, lower, x)
    return cdf_val
end

function get_sample_from_rdm(rdm::Matrix, basis::Basis, d::Int)
    ...
    state, ϕ(x_k). 
    """
    Z = get_normalisation_constant(rdm, basis, d)
    # sample a uniform random value from U(0,1)
    u = rand()
    # solve for x by defining an auxilary function g(x) such that g(x) = F(x) - u
    cdf_wrapper(x) = get_cdf(x, rdm, Z, basis, d) - u
    sampled_x = find_zero(cdf_wrapper, basis.range; rtol=0)
    # map sampled x_k back to a state
    sampled_state = get_state(sampled_x, basis, d)
...
function check_inverse_sampling(rdm::Matrix, basis::Basis, d::Int)
    """Check the inverse sampling approach to ensure 
    that samples represent the numerical conditional
    probability distribution."""
    Z = get_normalisation_constant(rdm, basis, d)
    lower, upper = basis.range
    xvals = collect(lower:0.01:upper)
    probs = Vector{Float64}(undef, length(xvals))
    for (index, xval) in enumerate(xvals)
        prob = (1/Z) * get_conditional_probability(xval, rdm, basis, d)
        probs[index] = prob
    end
    return xvals, probs
end

function plot_samples_from_rdm(rdm::Matrix, basis::Basis, n_samples::Int, d::Int,
    ...
    @threads for i in eachindex(samples)
        samples[i], _ = get_sample_from_rdm(rdm, basis, d)
    end
    ...
    xvals, numerical_probs = check_inverse_sampling(rdm, basis, d)
...
function inspect_known_state_pdf(x::Float64, basis::Basis, d::Int, n_samples::Int; show_plot=true)
    ...
    rdm = state * state'
    h, samples = plot_samples_from_rdm(rdm, basis, n_samples, d, false)
    if show_plot
        display(h)
    end
    title!("Known value: $x")
...
function get_dist_mean_difference(eval_intervals::Int, basis::Basis, 
    d::Int, n_samples::Int)
...
function get_cpdf_mode(rdm::Matrix, basis::Basis, d::Int;
    dx = 1E-4)
    ...
    mode_state = get_state(mode_x, basis, d)

    return mode_x, mode_state

end

function get_cdf_mean_std(rdm::Matrix, basis::Basis, d::Int;
    dx = 1E-4)
    ...
    expect_x = (1/n) * dot(xvals, probs)
    squared_diffs = (xvals .- expect_x).^2
    var = (1/n) * dot(squared_diffs, probs)
    std_val = sqrt(var)

    expect_state = get_state(expect_x, basis, d)
