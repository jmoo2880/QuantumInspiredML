...
function any_interpolate_directMedian(
    ...
    for (ii,i) in enumerate(inds)
        ...
        mx, ms, mad = get_median_from_rdm(matrix(rdm), opts; binary_thresh=binary_thresh, get_wmad=wmad) # dx = 0.001 by default
...
function any_interpolate_ITS_single(
    ...
    for (ii,i) in enumerate(inds)
        ...
        sx, ss = get_sample_from_rdm(matrix(rdm), opts; atol=atol)
