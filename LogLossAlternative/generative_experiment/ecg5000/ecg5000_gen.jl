...
mps_loaded = loadMPS("LogLossAlternative/generative_experiment/ecg5000/chi35_mps.h5");
state0 = sliceMPS(mps_loaded, 0)
state1 = sliceMPS(mps_loaded, 1)
@load "/Users/joshua/Documents/QuantumInspiredML/LogLossAlternative/generative_experiment/ecg5000/X_test.jld2"
...
function plot_examples_c1(sample_idx, num_shots; num_tpts_forecast=50)
    ...
    start_site = 140 - num_tpts_forecast
    ...
    plot!(collect((start_site+1):140), mean_ts[(start_site+1):end], ribbon=std_ts[(start_site+1):end], label="MPS forecast", ls=:dot, lw=2, alpha=0.5)
    plot!(collect((start_site+1):140), c1_test_samples[sample_idx, (start_site+1):end], lw=2, label="Ground truth", alpha=0.5)
    xlabel!("Time")
    ylabel!("x")
...
function plot_examples_c0(sample_idx, num_shots; num_tpts_forecast=50)
    ...
    plot!(collect((start_site+1):140), mean_ts[(start_site+1):end], ribbon=std_ts[(start_site+1):end], label="MPS forecast", ls=:dot, lw=2, alpha=0.5)
    plot!(collect((start_site+1):140), c0_test_samples[sample_idx, (start_site+1):end], lw=2, label="Ground truth", alpha=0.5)
    xlabel!("Time")
    ylabel!("x")
