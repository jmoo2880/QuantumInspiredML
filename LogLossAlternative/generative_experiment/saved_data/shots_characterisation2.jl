...
mps_loaded = loadMPS("/Users/joshua/Documents/QuantumInspiredML/LogLossAlternative/generative_experiment/saved_data/chi50_mps.h5");
state0 = sliceMPS(mps_loaded, 0)
state1 = sliceMPS(mps_loaded, 1)
@load "/Users/joshua/Documents/QuantumInspiredML/LogLossAlternative/generative_experiment/saved_data/test.jld2"
...
function plot_examples_c1(sample_idx, num_shots; num_tpts_forecast=50)
    all_shots_forecast = Matrix{Float64}(undef, num_shots, 200)
    start_site = 200 - num_tpts_forecast
    ...
    plot!(collect((start_site+1):200), mean_ts[(start_site+1):end], ribbon=std_ts[(start_site+1):end], label="MPS forecast", ls=:dot, lw=2, alpha=0.5)
    plot!(collect((start_site+1):200), c1_test_samples[sample_idx, (start_site+1):end], lw=2, label="Ground truth", alpha=0.5)
    xlabel!("Time")
    ylabel!("x")
