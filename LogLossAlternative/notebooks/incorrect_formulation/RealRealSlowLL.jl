...
function compute_loss_per_sample_mps(mps::MPS, product_state::PState)
    # more general functionm to compute loss for entire MPS
    # exatract the label
    y_gt = product_state.label 
    ...
    label_index = findinds(yhat, "f(x)")[1]
    y = label_to_tensor(y_gt, label_index)
...
function compute_model_probabilities(yhat)
    ...
    return [abs(yhat[i])^2 / norm_val for i in 1:ITensors.dim(yhat)]
...
function get_prediction_single(mps::MPS, product_state::PState)
    ...
    prediction = argmax(probas) - 1 # convert index back to label in one-hot encoding scheme
...
function generate_toy_timeseries(time_series_length::Int, total_dataset_size::Int, 
    ...
    A2, f2, sigma2 = 1.0, 3.0, 0.0 # Class 1
...
function train_mps(seed::Int=42)
    ...
    (X_train, y_train), (X_test, y_test) = generate_toy_timeseries(100, 500; plot_examples = true)
    ...
    return mps, losses, pstates, test_acc_per_sweep, data
