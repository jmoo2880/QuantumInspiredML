...
function get_probability_density_target(x::Float64, rdm::Matrix)
    """Evaluate the probability density for the target distribution."""
...
function forecast_mps_sites_importance(label_mps::MPS, known_values::Vector{Float64}, forecast_start_site::Int)
    ...
    for i in forecast_start_site:length(mps)
        ...
        sampled_x, sampled_state = sample_state_from_proposal(rdm_matrix)
        x_samps[i] = sampled_x
        if i != length(mps)
